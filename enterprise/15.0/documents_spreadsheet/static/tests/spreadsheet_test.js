...
module(
    ...
    function () {
        ...
        test("collaborative session client has the user id", async function (assert) {
            const { model } = await createSpreadsheet({
                data: this.data,
            });
            const localClientId = model.session.clientId;
            const localClient = model.session.clients[localClientId];
        ...
        test("Insert missing pivot value give the focus to the hidden input when model is closed", async function (assert) {
            assert.expect(2);
