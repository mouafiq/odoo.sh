...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_error_archive_partner_with_active_sub(self):
        """
        Cannot archive partner that has an active subscription
        in progress as an invoice or shipping address
        """
        self.sale_order.order_line.mapped('subscription_id').unlink()
        self.sale_order.action_confirm()
        subscription = self.sale_order.order_line.mapped('subscription_id')
        ...
        self.assertEqual(subscription.stage_category, "progress",
                         "Creating the first invoice for the subscription should set it's state to \"progress\".")
        with self.assertRaises(ValidationError):
            self.partner_a_invoice.action_archive()
        with self.assertRaises(ValidationError):
            self.partner_a_shipping.action_archive()
