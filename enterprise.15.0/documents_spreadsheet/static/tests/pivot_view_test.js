...
import PivotCache from "@documents_spreadsheet/js/o_spreadsheet/helpers/pivot_cache";
...
test("pivot with many cells don't expand subgroups", async function (assert) {
    ...
    assert.strictEqual(Object.keys(model.getters.getCells(sheetId)).length, 16);
    assert.strictEqual(getCellFormula(model, "A3"), '=PIVOT.HEADER("1","bar","false")');
    ...
    assert.strictEqual(getCellFormula(model, "A8"), '=PIVOT.HEADER("1")');
    assert.strictEqual(getCellFormula(model, "B1"), '=PIVOT.HEADER("1")');
    assert.strictEqual(getCellFormula(model, "B2"), '=PIVOT.HEADER("1","measure","__count")');
    assert.strictEqual(getCellFormula(model, "B3"), '=PIVOT("1","__count","bar","false")');
    assert.strictEqual(
        getCellFormula(model, "B4"),
        '=PIVOT("1","__count","bar","false","product_id","41")'
    );
    assert.strictEqual(getCellFormula(model, "B5"), '=PIVOT("1","__count","bar","true")');
    assert.strictEqual(
        getCellFormula(model, "B6"),
        '=PIVOT("1","__count","bar","true","product_id","37")'
    );
    assert.strictEqual(
        getCellFormula(model, "B7"),
        '=PIVOT("1","__count","bar","true","product_id","41")'
    );
    assert.strictEqual(getCellFormula(model, "B8"), '=PIVOT("1","__count")');
